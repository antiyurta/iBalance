...
interface IProps {
  ...
  type: DataIndexType;
  isActive?: boolean;
  filter: (key: any, state: boolean) => void;
...
const getColumnSearchProps = (props: IProps) => ({
  filterDropdown: ({
    setSelectedKeys,
    selectedKeys,
    confirm,
    ...
    const { label, dataIndex, type, isActive, filter, handleSearch } = props;
    const filterData: CheckedList = filters?.map((filter) => filter.value);
    const [searchValue, setSearchValue] = useState<string | number>("");
    const [checkedList, setCheckedList] = useState<CheckedList>([]);
    let indeterminate =
      checkedList.length > 0 && checkedList.length < filterData.length;
    const onCheckAllChange = (e: any) => {
      setCheckedList(e.target.checked ? filterData : []);
    };
    ...
    // search hiihes omno buh nohtsolig shaglah
